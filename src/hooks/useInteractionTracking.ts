...
export const useInteractionTracking = () => {
  ...
  const trackInteraction = useCallback((data: InteractionData) => {
    const { type, elementId, elementClass, elementText, elementType, value, success, error, metadata } = data;
  ...
  const trackButtonClick = useCallback((element: HTMLButtonElement | HTMLAnchorElement | null, metadata?: Record<string, any>) => {
    if (!element) return;
  ...
  const trackFormSubmit = useCallback((form: HTMLFormElement | null, success: boolean, error?: string) => {
    if (!form) return;
  ...
  const trackFormInput = useCallback((input: HTMLInputElement | null, value: string) => {
    if (!input) return;
  ...
  // Track menu toggles
  const trackMenuToggle = useCallback((isOpen: boolean, menuType: string) => {
    trackInteraction({
      type: 'menu_toggle',
      metadata: {
        menu_type: menuType,
  ...
  // Track copy actions
  const trackCopy = useCallback((text: string, success: boolean) => {
    trackInteraction({
      type: 'copy',
      success,
      metadata: {
  ...
  const trackDownload = useCallback((filename: string, fileType: string, success: boolean) => {
    trackInteraction({
      type: 'download',
      success,
      metadata: {
